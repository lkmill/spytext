...
assign(p, 'spy', function() {
	var that = this;
	function change(mutation) {
		that.dispatchEvent(new CustomEvent('change', { bubbles: true , details: { mutation: mutation }}));
		that.unspy();
	}
	...
	this.mutationObserver.observe(this, { subtree: true, characterData: true, childList: true, attributes: true });
});
assign(p, 'unspy', function() {
	if(this.mutationObserver) {
