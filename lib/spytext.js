...
var SpytextField = function(spytext, element, config) {
	...
	$(element).addClass('spytext-field');
					...
					case 46:
						...
						if(rng.collapsed) {
							...
							if(e.keyCode === 8 && positron.start.offset === 0) {
								...
								if(prev) {
									this.spytext.actions.join.call(this.spytext, $(prev).is('UL, OL') ? prev.lastChild : prev, block);
								}
							} else if(e.keyCode === 46 && positron.start.offset === positron.start.ref.textContent.length) {
								e.preventDefault();
								var next = block.nextSibling;
								if(next) {
									this.spytext.actions.join.call(this.spytext, block, $(next).is('UL, OL') ? next.firstChild : next);
								}
							}
						}
