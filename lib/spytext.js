...
var Spytext = function(element, config) {
	...
	//if(element) {
		...
		//	element.M('[data-spytext-field], [spytext-field], .spytext-field').each(function() {
...
Spytext.prototype = {
	...
	//	var field = this.addField(element);
	//	field.focus();
	//},
	events: {
		doc: {
...
SpytextField.prototype = {
	...
	change: function(mutation) {
		console.log(mutation);
		this.element.dispatchEvent(new CustomEvent('change', { bubbles: true }));
	},
	focus: function() {
		this.element.focus();
		var pos = { ref: this.element, offset: this.element.textContent.length, isAtStart: false };
		positions = {
			start: pos,
			end: pos
		};
		this.selectron.positron(positions,this.selectron).restore();
	},
	activate: function() {
		this.spytext.setCurrentField(this);
		this.element.spy();
		//console.log('activate');
		//this.mo.observe(this.element, { subtree: true, characterData: true });
	},
	deactivate: function() {
	...
	events: {
		...
		blur: function () {
			...
			this.deactivate();
			//if(this.element.innerHTML !== this.originalValue) {
