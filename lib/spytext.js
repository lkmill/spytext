...
var Spytext = function(element, config) {
	...
	for(var property in this.events.doc) 
		document.bind(property, this.events.doc[property], this);
...
Spytext.prototype = {
	...
	actions: {
		...
		block: function(options) {
			...
			var blocks = [];
			this.currentField.selectron.contained(1, 1).each(function(){
				var child = this;
			...
			if(commonAncestor === this.currentField.element || (commonAncestor.nodeType === 1 && commonAncestor.matches('UL, OL'))) {
				...
				if(this.currentField.selectron.contained(M(startBlock), null, true).length > 0) {
					...
					if(startBlock.firstChild.tagName === 'LI') {
						this.currentField.selectron.contained(startBlock.childNodes, null, true).vanish();
				...
				if(endBlock && this.currentField.selectron.contained(M(endBlock), null, true).length < 1) {
					...
					if(endBlock.firstChild.tagName === 'LI') {
						...
						this.currentField.selectron.contained(endBlock.childNodes, null, true).vanish();
					} else block = endBlock;
	...
	execute: function(action, options) {
		...
		if(this.actions[action]) {
			...
			setTimeout(function() {
				that.currentField.snapback.register();
			}, 100);
		}
	}
};
...
var SpytextDropdown = function(config, spytext) {
	...
	this.config.items.forEach(function(item) {
		...
		for(property in that.events.item) 
			el.bind(property, that.events.item[property], that);
		that.items.push(item);
	});

	for(property in this.events.dropdown) 
		that.element.bind(property, this.events.dropdown[property], this);
		...
		var index = (function(items) {
			for(var j = 0; j < items.length; j++) {
				for(var i in options) {
					if(options[i] === items[j].options[i])
						return j;
				}
			}
		})(this.items);
...
SpytextToolbar.prototype = {
	...
	setActiveStyles: function() {
			...
			switch(tag) {
				...
				case 'H6':
					...
					if(that.dropdowns.type) {
						console.log('has that.dropdowns.type');
		...
		console.log('setting active styles');
		var that = this;
		var tags = [];
		var blocks = [];
		var rng;
...
var SpytextField = function(spytext, element, config) {
	...
	for (var property in this.events)
		element.bind(property, this.events[property], this);
};
