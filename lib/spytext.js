	...
	actions: {
		align: function(options) {
			...
			containedChildren.forEach(function(child) {
				if(!child.matches('ul, ol')) $(child).css('text-align', options.alignment);
			});
		},
		...
		join: function(node1, node2) {
			...
			if(node1.matches('LI') && node2.matches('LI') && $(node1).closest('UL,OL')[0] !== $(node2).closest('UL, OL')[0]) {
		...
		newline: function() {
			...
			var block = rng.startContainer.nodeType === 1 && rng.startContainer.matches(blockTags.join(',')) ? rng.startContainer : $(rng.startContainer).closest(blockTags.join(','), this.currentField.element)[0];
					...
					case 46:
						...
						if(rng.collapsed) {
							var block = element.childNodes.includes(rng.startContainer) ?
								rng.startContainer :
								rng.startContainer.closest(blockTags.join(','), element);
							...
							if(e.keyCode === 8 && positron.start.offset === 0) {
								...
								if(prev) {
									this.spytext.actions.join.call(this.spytext, prev.matches('UL, OL') ? prev.lastChild : prev, block);
								}
							} else if(e.keyCode === 46 && positron.start.offset === positron.start.ref.textContent.length) {
								e.preventDefault();
								var next = block.nextSibling;
								if(next) {
									this.spytext.actions.join.call(this.spytext, block, next.matches('UL, OL') ? next.firstChild : next);
								}
							}
						}
