...
console.log(blockTags.join(','));
[ 'indexOf', 'slice', 'forEach' ].forEach(function(method) {
...
function ancestors(element, ufo, stop) {
	var nodes = [];
	var $node = $(element).closest(ufo, stop);
	while($node.length > 0) {
		nodes.push($node[0]);
...
var Sayer = function(element, config, toolbar) {
	this.fields = [];
	...
	if(toolbar) {
		...
		this.toolbar = new SayerToolbar({ preset: 'standard' }, this);
...
Sayer.prototype = {
	...
	actions: {
		align: function(options) {
			...
			containedChildren.forEach(function(child) {
				if(!$(child).is('ul, ol')) $(child).css('text-align', options.alignment);
			});
		},

		block: function(options) {
			function block(node, insert) {
		...
		deleteRangeContents: function(rng) {
			...
			if(commonAncestor === this.currentField.element || (commonAncestor.nodeType === 1 && $(commonAncestor).is('UL, OL'))) {
				var positron = this.currentField.selectron.get();
				var firstNode, lastNode;

				var startAncestors = rng.startContainer.ancestors(null, this.currentField.element);
				...
				if(this.currentField.selectron.contained(startBlock, null, true).length > 0) {
					startBlock.empty();
				} else {
				...
				var endAncestors = ancestors(rng.endContainer, null, this.currentField.element).toArray();
		...
		newline: function() {
			...
			var block = rng.startContainer.nodeType === 1 && $(rng.startContainer).is(blockTags.join(',')) ? rng.startContainer : $(rng.startContainer).closest(blockTags.join(','), this.currentField.element)[0];
			...
			if($(block).is('LI') && block.textContent.length === 0) {
				...
				this.currentField.selectron.set($el[0], 0);
			}
		},
		...
		format: function(options){
			var positron = this.currentField.selectron.get(null, true);
			var containedTextNodes = this.currentField.selectron.contained(3);
			var rng = this.currentField.selectron.range();
			...
			$(containedTextNodes).wrap(options.container ? $(options.container).clone() : $('<' + options.tag + '></' + options.tag + '>'));
			...
			this.currentField.selectron.contained(1, 1).forEach(function(contained) {
				contained.normalize();
			});
		...
		removeFormat: function() {
			...
			this.currentField.element.normalize();
		}
	},

	removeField: function(ufo) {
		if(ufo instanceof SayerField) {
	...
	addField: function(element, config) {
		...
		var field = new SayerField(this, element, config);
...
var SayerButton = function(config, sayer) {
	this.sayer = sayer;
	this.config = typeof config.preset === 'string' ? merge(this.presets[config.preset], config) : config;
	this.element = $('<button class="sayer-button" st-button-type="' + config.preset + '" tabindex="' + -1 + '">')[0];
...
SayerButton.prototype = {
	...
	events: {
		click: function(e) {
			e.preventDefault();
			this.sayer.execute(this.config.action, this.config.options);
		}
	},
...
var SayerDropdown = function(config, sayer) {
	this.sayer = sayer;
	this.config = typeof config.preset === 'string' ? merge(this.presets[config.preset], config) : config;
	this.items = [];
	this.element = $('<ul class="sayer-dropdown">')[0];
		...
		item: {
			click: function(e) {
				if($(this.element).hasClass('expanded')) {
					...
					this.sayer.execute(this.config.action, this.items[index].options);
				}
			}
		}
	},
...
var SayerToolbar = function(config, sayer) {
	this.dropdowns = [];
	this.buttons = [];
	this.sayer = sayer;
	this.config = typeof config.preset === 'string' ? merge(this.presets[config.preset], config) : config;
	this.element = $('<div class="sayer-toolbar">')[0];
...
SayerToolbar.prototype = {
	addButtonGroup: function(group) {
		var $ul = $('<UL>').addClass('sayer-button-group ' + group.name);
		group.buttons.forEach(function(buttonType) {
			var $li = $('<LI>');
			var button = this.addButton({ preset: buttonType }, this.sayer);
			$ul.append($li);
	...
	addButton: function(config) {
		var button = new SayerButton(config, this.sayer);
	...
	addDropdown: function(config) {
		...
		var dropdown = new SayerDropdown(config, this.sayer);
	...
	setActiveStyles: function() {
		...
		while(!rng) {
			rng = this.sayer.currentField.selectron.range();
			if(s++ > 10000) return;
		}
		...
		if(rng.collapsed) {
			tags = ancestors(rng.startContainer, null, this.sayer.currentField.element);

			// The last element should always be the block element, such as P or UL.
		} else {
			var el = rng.commonAncestorContainer;
			tags = ancestors(el, null, this.sayer.element);
		}
...
var SayerField = function(sayer, element, config) {
	this.sayer = sayer;
	this.element = element;
	$(element).addClass('sayer-field');
	...
	this.snapback = new Snapback(element, { preset: 'sayer', selectron: this.selectron});

	for (var property in this.events)
		$(element).on(property, this.events[property].bind(this));
};

SayerField.prototype = {
	clearTextNodes: function() {
		var children = [];
		console.log(this.element.childNodes.length);
		for(var i = 0; i < this.element.childNodes.length; i++) {
			children.push(this.element.childNodes[i]);
		}
	...
	activate: function() {
		...
		this.observe();
		this.active = true;
		this.sayer.setCurrentField(this);
	},
	...
	events: {
		...
		mouseup: function(e) {
			this.sayer.toolbar.setActiveStyles();
		},
			...
			if (e.ctrlKey) {
				...
				switch(e.keyCode) {
					...
					case 85://u
						...
						arr[66] = 'bold';
						arr[85] = 'underline';
						this.sayer.execute('format', { command: arr[e.keyCode] }, this.element);
						break;
					case 89://y
						e.preventDefault();
						this.snapback.redo();
				...
				if(inbetween(33,40)) {
					...
					this.snapback.register();
					this.sayer.actions.deleteRangeContents.call(this.sayer, rng);
				...
				switch(e.keyCode) {
					...
					case 46:
						// 8: backspace
						// 46: delete
						var selectron = this.selectron, element = this.element;
						if(rng.collapsed) {
							var check = blockTags.join(',');
							var block = element.childNodes.includes(rng.startContainer) ?
								rng.startContainer :
							...
							if(e.keyCode === 8 && positron.start.offset === 0) {
								...
								if(prev) {
									this.sayer.actions.join.call(this.sayer, $(prev).is('UL, OL') ? prev.lastChild : prev, block);
								}
							} else if(e.keyCode === 46 && positron.start.offset === positron.start.ref.textContent.length) {
								e.preventDefault();
								var next = block.nextSibling;
								if(next) {
									this.sayer.actions.join.call(this.sayer, block, $(next).is('UL, OL') ? next.firstChild : next);
								}
							}
						}
					...
					case 13:
						//enter
						e.preventDefault();
						this.sayer.actions.newline.call(this.sayer);
						break;
				}
			}
		},
		...
		paste: function (e) {
			...
			this.sayer.execute('paste', e.clipboardData ? e.clipboardData : clipboardData);
